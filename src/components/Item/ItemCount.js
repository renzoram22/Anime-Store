...
const ItemCount = (props) => { 
    ...
    const onAdd = () => {
        setAddButtonContent(<div><i aria-hidden="true" class="spinner loading icon"></i></div>)
        if(stock >= numero){
            setTimeout(() => {
    ...
    return(
        <div className='ItemCountContainer'>
            ...
            <div class="ui input"><input value={numero} type="number" />
            </div>
            <button class="ui button"  onClick={handleIncrement}>
               <div class="visible content">+</div>
               </button>
               <button class="ui button addButton"  onClick={onAdd}>{addButtonContent}</button>
        </div>
    )
